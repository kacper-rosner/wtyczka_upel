...
const questionsStatusDiv = document.getElementById('questionsStatus');
...
chrome.storage.local.get(['contextEnabled', 'knowledgeModeEnabled', 'floatingBtn', 'hideButtons', 'hideLogs', 'lastAiResponse', 'pdfRead', 'customContextFile', 'learnedContext', 'hideOnPageLogs', 'savedQuestionLog'], (result) => {
    ...
    updateQuestionsStatus(result.savedQuestionLog);
});
...
chrome.storage.onChanged.addListener((changes, namespace) => {
    if (namespace === 'local') {
        ...
        if (changes.savedQuestionLog) {
            updateQuestionsStatus(changes.savedQuestionLog.newValue);
        }
    }
});
...
function updateQuestionsStatus(savedQuestionLog) {
    if (!questionsStatusDiv) return;
    const count = Array.isArray(savedQuestionLog) ? savedQuestionLog.length : 0;
    questionsStatusDiv.innerText = count ? `Zapisane pytania do TXT: ${count}` : 'Zapisane pytania do TXT: 0';
    questionsStatusDiv.style.color = count ? '#16a34a' : '#64748b';
    questionsStatusDiv.style.fontSize = '12px';
    questionsStatusDiv.style.fontWeight = 'bold';
    questionsStatusDiv.style.textAlign = 'center';
}
...
function getTxtFilename() {
    ...
    return `pytania_i_odpowiedzi_${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}_${pad(now.getHours())}-${pad(now.getMinutes())}.txt`;
}

if (downloadQuestionsBtn) {
    downloadQuestionsBtn.addEventListener('click', () => {
        chrome.runtime.sendMessage({ action: 'getSavedQuestionsTxt' }, (response) => {
            if (chrome.runtime.lastError) {
                questionsStatusDiv.innerText = 'Błąd pobierania TXT.';
                questionsStatusDiv.style.color = '#ef4444';
                return;
            }
            if (response && response.error) {
                questionsStatusDiv.innerText = 'Błąd: ' + response.error;
                questionsStatusDiv.style.color = '#ef4444';
                return;
            }
            downloadTextFile(getTxtFilename(), response?.text || 'Brak zapisanych pytań.');
            updateQuestionsStatus(new Array(response?.count || 0));
        });
    });
}
...
if (clearQuestionsBtn) {
    clearQuestionsBtn.addEventListener('click', () => {
        if (!confirm('Wyczyścić zapisane pytania z pliku TXT?')) return;
        chrome.runtime.sendMessage({ action: 'clearSavedQuestions' }, (response) => {
            if (response && response.error) {
                questionsStatusDiv.innerText = 'Błąd: ' + response.error;
                questionsStatusDiv.style.color = '#ef4444';
                return;
